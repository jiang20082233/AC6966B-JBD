...
u32 file_bs_open_handle(FILE_BS_DEAL *fil_bs, u8 *ext_name)
{
    ...
    printf("fil_bs->dev = %x\n", fil_bs->dev);
    printf("%s, %s, %s, %d\n]", dev_manager_get_bs_root_path(fil_bs->dev), dev_manager_get_logo(fil_bs->dev), __FUNCTION__, __LINE__);
    fset_ext_type(dev_manager_get_bs_root_path(fil_bs->dev), ext_name);
