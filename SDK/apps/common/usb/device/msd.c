...
#include "system/includes.h"
struct umsd_sync_info {
    volatile u8 rw : 1;
    volatile u8 flag : 1;
    volatile const char *task;
    u16 lba;
    u16 num;
    void *buf;
    int err;
};
static struct umsd_sync_info umsd_sync_info;
#endif
//               Mass Storage Class
    /*********************************************************/
...
static u32 msd_usb2mcu(const struct usb_device_t *usb_device, u8 *buffer, u32 len)
{
    usb_dev usb_id = usb_device2id(usb_device);
    return usb_g_bulk_read(usb_id, MSD_BULK_EP_OUT, buffer, len, 1);
}
static u32 msd_mcu2usb(const struct usb_device_t *usb_device, const u8 *buffer, u32 len)
{
    usb_dev usb_id = usb_device2id(usb_device);
...
void USB_MassStorage(const struct usb_device_t *usb_device)
{
    ...
    if (private_scsi_cmd(&(msd_handle->cbw))) {
