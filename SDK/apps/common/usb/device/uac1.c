...
#if MIC_CHANNEL == 2
    0x03, 0x00, //wChannelConfig:Left Front
#else
...
static const u8 uac_mic_feature_desc[] = {
    ...
    0x01,      //ControlSize:1 byte
    0x01,      //bmaControls:Master Mute
    0x02,       //Controls(0):Left Volume
#if MIC_CHANNEL == 2
    0x02,       //Controls(0):Right Volume
#endif
    0x00,       //Feature String
};
///USB Microphone OT
...
/* u16 uac_get_cur_vol(const usb_dev usb_id) */
void uac_get_cur_vol(const usb_dev usb_id, u16 *l_vol, u16 *r_vol)
{
    ...
    if (uac_info) {
        if (!uac_info->spk_mute) {
            if (l_vol) {
                *l_vol = uac_info->spk_left_vol;
            }
            if (r_vol) {
                *r_vol = uac_info->spk_right_vol;
            }
        }
    }
}
    ...
    case UAC_FU_MUTE:
        if (HIBYTE(setup->wIndex) == SPK_FEATURE_UNIT_ID) {
            ...
            if (mute) {
                ...
                uac_mute_volume(SPK_FEATURE_UNIT_ID, uac_info->spk_left_vol, uac_info->spk_right_vol);
                uac_info->spk_mute = 0;
            }
            ...
            if (mute) {
                ...
                uac_mute_volume(MIC_FEATURE_UNIT_ID, uac_info->mic_vol, 0);
                uac_info->mic_mute = 0;
            }
        }
        break;
...
static u32 audio_ac_itf_handler(struct usb_device_t *usb_device, struct usb_ctrlrequest *setup)
{
    ...
    case UAC_GET_CUR:
        ...
        break;
    case UAC_GET_DEF:
        tx_payload = (u8 *)&uac_info->def_vol;
        tx_len = setup->wLength;
        usb_set_data_payload(usb_device, setup, tx_payload, tx_len);
        break;
    case UAC_GET_MAX:
        tx_payload = (u8 *)&uac_info->max_vol;
        tx_len = setup->wLength;
        usb_set_data_payload(usb_device, setup, tx_payload, tx_len);
        break;
    case UAC_GET_MIN:
        tx_payload = (u8 *)&uac_info->min_vol;
        tx_len = setup->wLength;
        usb_set_data_payload(usb_device, setup, tx_payload, tx_len);
        break;
    case UAC_GET_RES:
        tx_payload = (u8 *)&uac_info->vol_res;
        tx_len = setup->wLength;
        usb_set_data_payload(usb_device, setup, tx_payload, tx_len);
        break;
    case UAC_GET_LEN:
...
static void open_mic(struct usb_device_t *usb_device)
{
    ...
    uac_mic_stream_open(MIC_AUDIO_RATE, MIC_FRAME_LEN, MIC_CHANNEL);
...
u32 uac_register()
{
    ...
    uac_info->spk_left_vol = uac_get_spk_vol();
    uac_info->spk_right_vol = uac_get_spk_vol();
    uac_info->max_vol = 99;
    uac_info->min_vol = 0;
    uac_info->def_vol = 49;
    uac_info->vol_res = 1;
    uac_info->mic_vol = 13 / 14.0 * 100;
    uac_info->spk_mute = 0;
    uac_info->mic_mute = 0;
    return 0;
}
