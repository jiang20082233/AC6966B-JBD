...
static void timer_add_update(void *p)
{
    ...
    if (ui_get_disp_status_by_id(CLOCK_TIME_YMD) > 0) {
        ...
        /* printf(">>%s %d\n",__FUNCTION__,ui_get_disp_status_by_id(CLOCK_TIME_YMD)); */
    }
...
static u8 __alm_set_reset(void)
{
    ...
    alm_set_table[0].time_value = alarm.time.bHour;
    alm_set_table[1].time_value = alarm.time.bMin;
...
static int menu_sys_alm_set()
{
    u8 index = 0;
    T_ALARM alarm;
    alarm.time.bHour = alm_set_table[0].time_value ;
    alarm.time.bMin  = alm_set_table[1].time_value;
