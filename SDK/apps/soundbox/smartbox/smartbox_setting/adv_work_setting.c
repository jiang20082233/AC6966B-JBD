...
{
    *work_setting_info = _s_info.work_mode;
}
...
static void adv_work_setting_sync(u8 *work_setting_info)
{
    ...
    if (get_bt_tws_connect_status()) {
        update_smartbox_setting(BIT(ATTR_TYPE_WORK_MODE));
    }
#endif
}
...
static void deal_work_setting(u8 *work_setting_info, u8 write_vm, u8 tws_sync)
{
    ...
    if (write_vm) {
        adv_work_setting_vm_value(&_s_info.work_mode);
    }
    if (tws_sync) {
        adv_work_setting_sync(&_s_info.work_mode);
    }
