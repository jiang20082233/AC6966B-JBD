...
#define DONGLE_USE_PCM_DEC				0 //使用单独的pcm解码做节拍
        ...
        struct audio_data_frame *out)
{
    ...
    if (p_dongle && p_dongle->start) {
        len = cbuf_write(&p_dongle->pcm_cbuf, in->data, len);
...
void audio_dongle_emitter_init(void)
{
    ...
    dongle_emitter.mixer.stream = audio_stream_open(NULL, audio_last_out_stream_resume);
