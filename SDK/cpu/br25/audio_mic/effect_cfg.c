...
{
    return &__this->EQ_Coeff_table[EQ_SECTION_MAX * 5 + mode * 5];
}
...
bool effect_cfg_open(struct __effect_mode_cfg *parm)
{
    ...
    for (int i = 0; i < __this->mode_cfg->mode_max; i++) {
        __this->mode[i].eq_design_mask = (u32) - 1;
    }
    ...
    if (effect_cfg_get_file_info()) { //获取EFFECTS文件失败
        ...
        for (int i = 0; i < __this->mode_cfg->mode_max; i++) {
            ...
            cur_mode->seg_num = EQ_SECTION_MAX;
            /*if (cur_mode->seg_num > ARRAY_SIZE(eq_tab_normal)) {
            	cur_mode->seg_num = ARRAY_SIZE(eq_tab_normal);
            }*/
            cur_mode->global_gain = 0;
            /* log_i("seg_num %d, default %d\n", cur_mode->seg_num, EQ_SECTION_MAX_DEFAULT); */
            for (int j = 0; j < cur_mode->seg_num; j++) {
                memcpy(&cur_mode->parm.eq.val.seg[j],
                       &eq_tab_normal[0],
                       sizeof(struct eq_seg_info));//前五段默认系数表填直通

            }
            /*memcpy(cur_mode->parm.eq.val.seg,
            	   eq_tab_normal,
            	   sizeof(struct eq_seg_info) * cur_mode->seg_num);//前五段默认系数表填直通
            */
            cur_mode->eq_design_mask = (u32) - 1;
        }
    }
