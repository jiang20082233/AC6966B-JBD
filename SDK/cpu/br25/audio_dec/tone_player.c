...
*/
/*----------------------------------------------------------------------------*/
static int tone_play_open_with_callback_base(const char **list, u8 follow, u8 preemption, void (*evt_handler)(void *priv, int flag), void *evt_priv)
{
    if (follow && tone_dec) {
...
static void tone2tws_rx_callback_func(u32 dat_addr)
{
    ...
    single_file[1] = NULL;
    tone_play_open_with_callback_base(single_file, 0, 1, NULL, NULL);
    free((void *)dat_addr);
}
...
#endif
    return tone_play_open_with_callback_base(list, follow, preemption, evt_handler, evt_priv);
}
